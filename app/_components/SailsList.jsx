import { getSails } from "@/app/_lib/data-service";
import { auth } from "@/app/_lib/auth";
import SailCard from "@/app/_components/SailCard";

async function SailsList({ filter }) {
  const session = await auth();
  const eqData = await getSails(session.user.appUserId);
      ...
      {eqData.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {displayedEq.map((equipment) => (
            <SailCard key={equipment.id} equipment={equipment} />
          ))}
        </div>
      )}
    </div>
